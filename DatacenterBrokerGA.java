...
public class DatacenterBrokerGA extends SimEntity {
    ...
    protected void submitCloudlets() {
        ...
        double fitness_threshold=9999.00;
        double makespans2[] = new double[50];

        //Genetic Algorithm
        //INITIAL POPULATION
        //first 20 chromosomes are made as random
        Random rand = new Random();
        int[][] a = new int[100][100];
        double probc=1,probm=1;
        int cno = 20;
        ...
        double OR[][] = new double[80][30];
        double throughput;
        // Fitness
        for (int i = 0; i < getCloudletList().size(); i++) {
            for (int j = 0; j < getVmsCreatedList().size(); j++) {
                OR[i][j] = cllen[i] / vmmips[j];
            }
        }
        int k = -1, w1 = 0;
        double fitness[] = new double[getCloudletList().size()];
        double maxfitness[] = new double[cno];
        for (int i = 0; i < cno; i++) {
            maxfitness[i] = 0.0;
        }
        ...
        for (int m = 0; m < cno; m++) {
            ...
            w1 = 0;
            maxfitness[k] = find_max(fitness);
        } 
        System.out.println("MAKESPANS");
        for (int w = 0; w < cno; w++) {
            System.out.println(" " + w + "  " + maxfitness[w]);
        }

        for (int w = 0; w < cno; w++) {
            makespans2[w] = maxfitness[w];
        }
     ...
     // calculate variance and load on each machine
        double VOR=0,Sum=0,AOR;
        for (int i = 0; i < getCloudletList().size(); i++) {
            for (int j = 0; j < getVmsCreatedList().size(); j++) {
                Sum=+OR[i][j];
            }
        }
        AOR=Sum/(getCloudletList().size()*getVmsCreatedList().size());
        for (int i = 0; i < getCloudletList().size(); i++) {
            for (int j = 0; j < getVmsCreatedList().size(); j++) {
                VOR=+(OR[i][j]-AOR)*(OR[i][j]-AOR);  
            }
        }
        fitnes=1/Math.sqrt(VOR);
        for(k=0;k<cno;k++) {
        if(fitnes<maxfitness[k]) {
        	probc=.9;
        	probm=0.05;
        }
        else {
        	probc=0.9*(maxfitness[k]-fitness[k])/(maxfitness[k]-fitnes);
        	probc=0.05*(maxfitness[k]-fitness[k])/(maxfitness[k]-fitnes);
        }
        }
        ...
        while (!(Math.abs(min_one) == Math.abs(current_min_makespan) && count_same == 3)) {
            ...
            for (int m = 0; m < cno; m = m + 2) {
                for (int n = 0, o = getVmsCreatedList().size(); n < getVmsCreatedList().size() && o < getCloudletList().size(); n++, o++) {
                	if(random.nextInt(100)<(probc*100)) {
                    temp1 = b[m][n];
                    b[m][n] = b[m + 1][o];
                    b[m + 1][o] = temp1;
                	}
            ...
            if (countm > 5) {
                ...
                for (int m = 0; m < cno; m++) {
                	if(random.nextInt(100)<(probm*100)) {
        ...
        System.out.println("\nSCHEDULED CLOUDLETS BASED IMPROVED GENETIC ALGORITHM\nCLOUDLETID\t\tVMID");
    ...
    public void shutdownEntity() {
        Log.printLine( "Data Center is shutting down...");
    }
